...
| `worldflow.html` 세상 흐름 파악 | [주요 일정] 다가오는·최근 종료 행사 + 월별 캘린더(접힘) / [핵심 인력] 키워드 검색·인물 디렉토리·주차별 발언 | `events.json`, `people.json` (반자동) |
| `earnings.html` 미국 주요 실적 정리 | 정리된 실적(요약·Q&A·전문) + 실적 캘린더 + 기업별 재무 열람(5개년/8분기 매출·YoY·영업이익·OPM, 분기는 QoQ) | `financials.json`(자동) + `earnings.json`(수동 큐레이션) + `megacap.json` |
...
- **실적 요약 등록**: `docs/earnings.json` — `earnings[]`에 `{ticker, name, quarter, report_date, summary(내용요약), qa(어닝콜 Q&A 요약), full(전문 분석 마크다운), tags}`. **수동 큐레이션** — 재무 수치(financials.json)는 자동이지만 실적 "내용·질문·전문"은 어닝콜 전문이 유료·저작권 콘텐츠라 자동 수집 불가. 반드시 내 언어로 요약(직접 인용 15단어 미만). 재무 열람은 earnings.json이 비어 있어도 financials.json만으로 동작함
...
- `earnings.json` — `earnings[]`: 실적 요약·Q&A·전문 (수동 큐레이션, 위 "수정 지점" 참조)
