...
| `megacap.html` 글로벌 메가캡 | 13개 섹터 강세 순위(중앙값), 섹터별 리더 TOP5, 300종 테이블 | `megacap.json`, `megacap_universe.json` (자동) |
| `perspective.html` 핵심 테제 트래킹 | 주차별 테제 카드 + 등록일 이후 평균 수익률(테제 스코어) | `theses.json` (수동) + `data.json`의 시세 |
...
python fetch_megacap.py     # 명단 시세 갱신 → docs/megacap.json (약 4분)
...
- `megacap.json` — `stocks[]`: `ticker/name/sector/mcap_usd/price/r1w/r1m/r3m/ytd/from_high/vol_ratio`
